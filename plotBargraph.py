    ...
    for alg in algorithms:
        ...
        for run in runs:
            try:
                ...
                print(values)
                values_arr.append(np.array([np.mean(a) for a in values]))
            except:
                print("No full data for run")
        transposed = [[values_arr[x][y] for x in range(len(values_arr))] for y in range(len(values_arr[0]))]
        boxes = transposed

    pyplot.boxplot(x=boxes)
