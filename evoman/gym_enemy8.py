...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self, dt, game):
        ...
        if ((abs(self.rect.left - game.player.rect.left) <= 200 or abs(
                self.rect.right - game.player.rect.right) <= 200)) and not self.gun_cooldown:
            atack6 = 1
        else:
            atack6 = 0
        ...
        if self.resting == 0 and self.just_shoot == 0 and atack6 == 1:
            ...
            for i in range(0, rand):
                self.twists.append(
                    Bullet_e8((self.rect.x + (i * 60), self.rect.y), i, self.direction, len(self.twists),
                              game.sprite_e))
...
class Bullet_e8(pygame.sprite.Sprite):
    ...
    def update(self, dt, game):
        ...
        if not (abs(self.rect.left - game.player.rect.left) <= 10 or abs(
                self.rect.right - game.player.rect.right) <= 10):
