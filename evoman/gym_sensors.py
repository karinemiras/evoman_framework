...
def get_sensors(game):
    ...
    for i in range(0, len(game.enemy.twists)):
        if game.enemy.twists[i] != None:
            posx_be = game.enemy.twists[i].rect.left + (
                        (game.enemy.twists[i].rect.right - game.enemy.twists[i].rect.left) / 2)
            posy_be = game.enemy.twists[i].rect.bottom + (
                        (game.enemy.twists[i].rect.top - game.enemy.twists[i].rect.bottom) / 2)
            param_values.append([posx_p - posx_be, posy_p - posy_be])
            # param_values.append()
    ...
    try:
        if game.inputscoded == "yes":
            types = struct.Struct(
                'q q q q q q q q q q q q q q q q q q q q')  # defines the data types of each item of the array that will be packed. (q=int, f=flo)
            ...
            coded_variables = [coded_variables[i:i + 2] for i in
                               range(0, len(coded_variables), 2)]  # breaks hexadecimal string in bytes.
