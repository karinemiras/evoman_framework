...
experiment_name = "multiswars_test"
...
creator.create("Particle", NeuralNetwork, fitness=creator.FitnessMax, speed=list,
...
def main(verbose=True):
    ...
    while mpb.nevals < 5e5:
        ...
        rexcl = (1 - (-1)) / (2 * len(population) ** (1.0 / NDIM))
        ...
        for swarm in population:
            # Check for change
            if swarm.best and toolbox.evaluate(env, individual=swarm.best) != swarm.bestfit.values:
            ...
            print(part.bestfit, swarm.bestfit)
            if part.fitness.values[0] >= 68:
                numpy.savetxt(experiment_name + '/multiswarm__part_gain_' + str(part.fitness.values[0]) + '.txt',
                              part.best)
            if swarm.bestfit.values[0] >= 68:
                numpy.savetxt(experiment_name + '/multiswarm_swarm_gain_' + str(swarm.bestfit.values[0]) + '.txt',
                              swarm.best)
