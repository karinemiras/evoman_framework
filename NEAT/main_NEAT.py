...
enemies = cfg["experiment_parameters"]["enemies"]
...
final_results = cfg["decision_variables"]["final_results"]
...
def run_experiment(env,  config, enemy, parameter_options = None, parameter_names=None):
    if tune_parameters:
        for parameters in parameter_options:
            config = change_config_file(config, parameter_names, parameters)
            for run in range(run_nr):
                apply_NEAT.run(env, generations, config, str(parameters)+ "_run_"+str(run), name_experiment, enemy)

    else:
        for run in range(run_nr):
            apply_NEAT.run(env, generations, config, run, name_experiment, enemy)
            apply_NEAT.evaluate_best_sol(env, run, name_experiment, enemy)
...
def main(config):
    ...
    for enemy in enemies:
        ...
        if apply_optimization and tune_parameters:
            run_experiment(env, config, enemy, parameter_options, parameter_names)

        if apply_optimization and final_results:
            run_experiment(env, config, enemy)
